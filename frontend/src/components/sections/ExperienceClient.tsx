...
export default function ExperienceClient({ work, education, certifications }: Props) {
  ...
  const dataMap: Record<Tab, Experience[]> = { work, education, certification: certifications };
        ...
        {items.length === 0 ? (
          ...
          <div className="flex gap-6 overflow-x-auto pb-6 -mx-6 px-6">
            {items.map((item) => (
              <div
                key={item.id}
                className="bg-gray-900/50 border border-gray-800 rounded-2xl p-6 hover:border-violet-500/50 transition-all duration-300 flex flex-col gap-4 shrink-0"
                style={{ width: "360px" }}
              >
                <div>
